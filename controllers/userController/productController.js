...
//       if (sort == 0) {
//           productData = await Product.find({ isAvailable:1, $and: [{ category: arr }, { $or: [{ name: { $regex: '' + search + ".*" } }, { category: { $regex: ".*" + search + ".*" } }] }] }).sort({$natural:-1}).skip(skip).limit(limit)
//       } else {
//           productData = await Product.find({ isAvailable:1, $and: [{ category: arr }, { $or: [{ name: { $regex: '' + search + ".*" } }, { category: { $regex: ".*" + search + ".*" } }] }] }).sort({ price: sort }).skip(skip).limit(limit)
//       }
...
const loadProducts = async (req, res) => {
  try {
      ...
      if (sort == 0) {
          ...
          productData = await Product.find({ isAvailable:1, $and: [{ category: arr }, { $or: [{ name: { $regex: '' + search + ".*" } }, { category: { $regex: ".*" + search + ".*" } }] }] }).sort({ price: sort })
          pageCount = Math.floor(productData.length/limit)
          if(productData.length%limit >0){
              pageCount +=1
          }
